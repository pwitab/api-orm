...
from api_orm.manager import Manager
from api_orm.utils import is_instance_or_subclass, import_string
...
class APIModelMeta(type):
    """Base metaclass for all APIModels"""

    def __new__(mcs, name, bases, attrs):

        # Also ensure initialization is only performed for subclasses of Model
        # (excluding Model class itself).
        parents = [b for b in bases if isinstance(b, APIModelMeta)]
        if not parents:
        ...
        schema_dict['__model__'] = new_class

        schema_klass = type(schema_name, (APISchema,), schema_dict)
        ...
        if endpoint is not None:
            ...
            api_klass_path = os.environ.get('API_ORM_API_CLASS',
                                            default='api_orm.api.NoAPI')
            api_klass = import_string(api_klass_path)
            manager.api = api_klass.load()

            new_class.objects = manager
            new_class.api = manager  # extra reference to manager
...
class APIModel(metaclass=APIModelMeta):
    id = None
