...
from api_orm.query import QueryCompiler, FilterParser
...
class VismaAPI:
    ...
    def __init__(self, client_id, client_secret, access_token, refresh_token,
                 token_expires, token_path=None, test=False):
    ...
    def post(self, endpoint, data, *args, **kwargs):
        ...
        if not r.ok:
            raise APIException(f'POST :: HTTP:{r.status_code}, {r.content}')
        return r
    ...
    def put(self, endpoint, data, **kwargs):
        ...
        if not r.ok:
            raise APIException(f'PUT :: HTTP:{r.status_code}, {r.content}')
        return r
    ...
    def delete(self, endpoint, **kwargs):
        ...
        if not r.ok:
            raise APIException(f'DELETE :: HTTP:{r.status_code}, {r.content}')
        return r
    ...
    def _refresh_token(self):
        ...
        if response.status_code != 200:
            raise APIException(f'Couldn\'t refresh token: '
    ...
    def load(cls):
        ...
        return cls(env['client_id'], env['client_secret'],
                   ...
                   token_expires=token_expires, token_path=env['token_path'],
                   test=env['test'])
